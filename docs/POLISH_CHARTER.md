...
4. **GLB files are gitignored.** `require('../assets/models/...')` calls are valid even though the files don't exist in the remote clone — Metro resolves them at build time on the user's machine. Don't "fix" them.
5. **Respect `CLAUDE.md`** conventions (Zustand selectors, GlossyButton web fallback, sprite sheet grids, etc.).
6. **Commit format:** `polish: <scope> — <one-line description>`. Co-Authored-By trailer for Claude.
7. **Stop after 1-3 commits.** Quality over quantity.
