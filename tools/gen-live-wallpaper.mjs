#!/usr/bin/env node
// Generates the live wallpaper asset set in parallel:
//   - bg-home (pure cosmic sky, no stage)
//   - nebula-back (deep starfield)
//   - nebula-mid (magenta + cyan cloud wisps)
//   - nebula-near (bright sparkle dust)
//
// ~$0.20 via Flux Pro v1.1. The nebula-mid and nebula-near layers also
// run through BiRefNet-style alpha handling — they're prompted to
// render on pure black so we can use mix-blend-mode: screen on web to
// composite them over nebula-back for a real layered parallax effect.
...
const TARGETS = ['bg-home', 'nebula-back', 'nebula-mid', 'nebula-near'];

const manifest = JSON.parse(fs.readFileSync(MANIFEST, 'utf-8'));
const items = TARGETS.map((id) => manifest.items.find((i) => i.id === id));
